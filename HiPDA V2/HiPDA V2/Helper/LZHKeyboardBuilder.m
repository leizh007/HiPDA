...
#import "LZHKeyboardTextKeyCell.h"
#import "LZHKeyboardPressedCellPopupView.h"
#import "UIImage+LZHHIPDA.h"

@implementation LZHKeyboardBuilder
...
+ (WUEmoticonsKeyboard *)sharedEmoticonsKeyboard {
    ...
    dispatch_once(&onceToken, ^{
        ...
        //Text keys
        NSArray *iconKeys = [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"EmotionList" ofType:@"plist"]];
        NSMutableArray *iconItems=[[NSMutableArray alloc]init];
        [iconKeys enumerateObjectsUsingBlock:^(NSDictionary *iconInfo, NSUInteger idx, BOOL *stop) {
            WUEmoticonsKeyboardKeyItem *keyItem = [[WUEmoticonsKeyboardKeyItem alloc] init];
            keyItem.image = [UIImage imageNamed:iconInfo[@"image"]];
            keyItem.textToInput = iconInfo[@"text"];
            [iconItems addObject:keyItem];
        }];
        
        NSArray *iconItemsArray=[[iconItems reverseObjectEnumerator]allObjects];
        
        //Icon key group
        WUEmoticonsKeyboardKeyItemGroup *imageIconsGroup = [[WUEmoticonsKeyboardKeyItemGroup alloc] init];
        imageIconsGroup.keyItems = iconItemsArray;
        ...
        textIconsLayout.itemSize = CGSizeMake([[UIScreen mainScreen]bounds].size.width/4, 142/3.0);
        ...
        textIconsGroup.keyItemCellClass = LZHKeyboardTextKeyCell.class;
        ...
        [keyboard setKeyItemGroupPressedKeyCellChangedBlock:^(WUEmoticonsKeyboardKeyItemGroup *keyItemGroup, WUEmoticonsKeyboardKeyCell *fromCell, WUEmoticonsKeyboardKeyCell *toCell) {
            [LZHKeyboardBuilder sharedEmotionsKeyboardKeyItemGroup:keyItemGroup pressedKeyCellChangedFromCell:fromCell toCell:toCell];
        }];
        ...
        if (textIconsLayout.collectionView) {
            ...
            UIImage *gridBgImage=[UIImage imageNamed:@"keyboard_grid_bg"];
            UIImage *resizedGridImage=[UIImage imageWithImage:gridBgImage scaledToSize:CGSizeMake([[UIScreen mainScreen]bounds].size.width, gridBgImage.size.height)];
            textGridBackgroundView.backgroundColor = [UIColor colorWithPatternImage:resizedGridImage];            [textIconsLayout.collectionView addSubview:textGridBackgroundView];
        }
...
{
    static LZHKeyboardPressedCellPopupView *pressedKeyCellPopupView;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        pressedKeyCellPopupView = [[LZHKeyboardPressedCellPopupView alloc] initWithFrame:CGRectMake(0, 0, 83, 110)];
