...
struct QuoteAuthorManager {
    static func quoteAuthor(fid: Int, tid: Int, pid: Int, content: String, success: PublishSubject<Int>, failure: PublishSubject<String>, disposeBag: DisposeBag) {
        NetworkUtilities.html(from: "/forum/post.php?action=reply&fid=\(fid)&tid=\(tid)&repquote=\(pid)&extra=page%3D1") { result in
            ...
            case let .success(html):
                do {
                    ...
                    HiPDAProvider.request(.quoteAuthor(fid: fid, tid: tid, pid: pid, formhash: formhash, noticeauthor: noticeauthor, noticetrimstr: noticetrimstr, noticeauthormsg: noticeauthormsg, content: content))
